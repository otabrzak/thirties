...
```{r showcase simple turn simulation}
turn()
```
...
```{r function for expected maxima}
e_max <- function(n){
    e = 0
    for (x in 1:6) {
        Fx=x/6
...
```{r more complex turn simulation}
...
3.  Our expected value from a single throw goes down with the ammount of dice in that throw. By this idea we can ignore all threshold vectors, in which any value in position i is greater than the value in position i-1.
...
# Create the plot
ggplot(top_10_vectors, aes(x = reorder(threshold_label, score), y = score)) +
...
```{r evolve, echo = TRUE, results = "hide"}
...
evolve <- function(n, n_epochs, sampled_turns, early_stop=5){
    ...
    p <- create_evolution_plot(evolution_progress = evolution_progress)
    print(p)
    ggsave("evolution_results_plot.png", plot = p, width = 8, height = 5)
    
    return(list("population"=population, "p"=p))
}
...
We found that there are 2 virtually identically performing optimal strategis for this game: $[6,6,6,5,5,4]$ and $[6,6,6,6,5,4]$. The brute force search with a very high number of simulations always converges near 1 of these options. However, the genetic algorithm uses a much lower number of simulations for scoring to for approximate scoring and speeding up the process. Because of that, the optimum found by the genetic algorithm may vary slightly and both algorithms will likely return different scores due to the difference of simulation length.
...
```
Since the brute force result was scored by a much higher number of turns, it will be muhc more precise than the GA solution. We can even calculate the 95% confidence interval for the true expected score using this strategy.
