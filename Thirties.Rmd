...
turn <- function(thresholds=c(5,5,5,5,5,5)) {
    ...
    while (dice_in_game > 0) {
        ...
        while (improvement_found) {
            ...
            current_threshold <- thresholds[length(throw)]
...
names(valid_vectors) <- paste0("V", 1:6)
...
for (i in 1:nrow(valid_vectors)) {
    ...
    current_quantiles <- quantile(sim_results)
    quantiles_list[[i]] <- current_quantiles
    
    
    cat("iteration", vec_i,"out of",nrow(valid_vectors), "\n")
    vec_i = vec_i+1
}

quantiles_df <- as.data.frame(do.call(rbind, quantiles_list))
quantiles_df$thresholds <- apply(valid_vectors,1, function(x) paste(x,collapse = ","))
...
make_population <- function(n) {
    ...
    population <- replicate(n, 0.5 + sample(0:5, 6, replace = TRUE))
...
evaluate_population <- function(population,pop_i){

    for (chrom_i in 1:nrow(population)) {
    
        chromosome <- as.double(population[chrom_i,1:6])
    
        sim_results <- replicate(1000,sum(turn(chromosome)))
        
        sim_score <- mean(sim_results)
        population$score[chrom_i] <- sim_score
        
        cat("Population", pop_i, "- chromosome", chrom_i,"\n")
        
    }
}

evaluate_population(population = population, pop_i = pop_i)
```
